<?php 
	...
	$sucursal = (isset($aViewData['sucursal'])) ? $aViewData['sucursal'] : null;
				...
				<div>
					...
					<div class="widget-body">
						...
						<div id="myTabContent3" class="tab-content padding-10">
							<h1>Marcantonio Deportes Web - <small>Resumen del pedido</small></h1>
							...
							<div class="row">
								...
								<div class="col col-sm-6"><strong>Dni:</strong> {{$item['dni']}}</div>
							</div>
							<hr />
							...
							<hr />
								<div class="row">
									...
									@if($item->etiqueta_envio)
										<div class="col col-sm-6"><a href="{{ $item->etiqueta_envio }}">Imprimir Etiqueta</a></div>
									@endif
									@if($item->imprimir_etiqueta)
										<div class="col col-sm-6"><a href="{{ $item->imprimir_etiqueta }}">Imprimir Etiqueta</a></div>
									@endif
								</div>
							@else
								<h4>Datos de entrega</h4>
								<div class="row">
									<div class="col col-sm-6"><strong>Sucursal:</strong> {{ isset($sucursal['nombre'])?$sucursal['nombre']:'' }}</div>
									<div class="col col-sm-6"><strong>Fecha de retiro:</strong> {{ isset($sucursal['fecha'])?$sucursal['fecha']:'' }}</div>
									<div class="col col-sm-6"><strong>Retira:</strong> {{ $item->nombre }}</div>
									<div class="col col-sm-6"><strong>DNI:</strong> {{ $item->dni}}</div>
									<div class="col col-sm-6"><strong>Telefono:</strong> {{ $item->telefono}}</div>
								</div>
							@endif
							...
							<div class="table-responsive">
								<table class="table table-bordered">
									...
									<tbody>
										...
										<tr>
											<td colspan="6" align="center"><strong>COMPRA NO FINALIZADA</strong></td>
										</tr>
										@else
{{--										<tr>
											<td align="right" colspan="6"><strong>SUBTOTAL</strong></td>
											<td align="right"><strong>{{$productos['moneda']}}{{$productos['subtotal']}}</strong></td>
										</tr>--}}
										<tr>
										...
										<tr>
											...
											<td align="right"><strong style="color:#f00;font-size:16px">{{$productos['moneda']}}{{$productos['total']}}</strong></td>
										</tr>
										@endif
									</tbody>
								</table>
						...
						<div class="text-center padding-10">
							<a id="imprimir_pedido" href="javascript:void(0);" class="btn btn-primary "><i class="fa fa-print"></i> Imprimir</a>
						</div>
					</div>
...
<script>
	$('#imprimir_pedido').on("click", function () {
		$('#myTabContent3').printThis({
			importCSS: true,            // import page CSS
			importStyle: true,         // import style tags
			base: "{{URL::to('/')}}/",               // preserve the BASE tag, or accept a string for the URL
		});
    });
</script>
