    ...
    $(document).ready(function() {
        ...
        var resourceTable = $('#' + resourceTableId).dataTable({
            ...
            "initComplete": function ()
            {
                ...
                var $selectPor = $('<select><option value="NULL">Registrados Por</option><option value="1" selected>WEB</option><option value="0">Mercado Libre</option></select>');

                $('.filtro-mas').append($selectPor);

                $selectPor.change(function(){
                    resourceTable.dataTable().fnFilter($(this).val(),1);
                });
                //end Registrados Por

                //Filtro Newsletter:
                var $selectNews = $('<select><option value="NULL" selected>Newsletter</option><option value="1">Suscriptos</option></select>');

                $('.filtro-mas').append($selectNews);
