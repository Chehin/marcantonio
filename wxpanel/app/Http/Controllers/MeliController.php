        ...
        if ($this->user->hasAccess('productos.create')) {
            ...
            if ($producto) {
                ...
                if(!$genero){
                    ...
                    $res = json_decode('{"body":{"message":"Para publicar en Mercado Libre se debe cargar el genero al producto","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                    SincroMeliLog::alta($res,$producto->id);
                    return response()->json($aResult);
                }else{
                ...
                if ($imagenes) {
                    ...
                    $res = json_decode('{"body":{"message":"Para publicar en Mercado Libre se debe cargar como minimo una foto - máximo 10 fotos","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                    SincroMeliLog::alta($res,$producto->id);
                    return response()->json($aResult);
                }
                ...
                if ($producto->categoria_variations || count($stocks) > 1) {
                        ...
                        foreach ($stocks as $stock) {
                            if ($stock->stock) {
                                ...
                                $variations[] = array(
                                    ...
                                    "available_quantity" => (int)$stock->stock,
                                    "price" => $precio->precio_db,
                    ...
                    if ($stocks[0]->stock == 0) {
                        ...
                        $res = json_decode('{"body":{"message":"Para publicar en Mercado Libre el producto debe tener un stock mayor a 0","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                        SincroMeliLog::alta($res,$producto->id);
                        return response()->json($aResult);
                    }
                ...
                SincroMeliLog::alta($result,$producto->id);
                $aResult['data'] = $result;
            } else {
                $aResult['status'] = 1;
                $aResult['msg'] = \config('appCustom.messages.itemNotFound');
                ...
                */
                ...
                if ('yes' === $request->input('justEnable')) {
                    ...
                    if ($status) {
                        ...
                        $res = json_decode('{"body":{"message":"No se pudo actualizar la publicación, intente nuevamente.","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                        SincroMeliLog::alta($res,$producto->id);
                    }
                ...
                if ($item_meli) {
                    ...
                    if ($producto->categoria_variations == 1) {
                            ...
                            foreach ($stocks_1 as $stock) {
                                if ($stock->stock>=0) {                                    
                                    ...
                                    if (count($imagenesColor) == 0) {
                                        $aResult['status'] = 1;
                                        $aResult['msg'] = "Debe cargar como mínimo una foto por cada color.";
                                        $res = json_decode('{"body":{"message":"Debe cargar como mínimo una foto por cada color","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                                        SincroMeliLog::alta($res,$producto->id);
                                        return response()->json($aResult);
                                    }
                        ...
                        if($freezed_by_deal == 1){
                                $variations = $this->updateVariationsByDeal($stocks_1, $variationsInit, $precio->precio_db, $producto->id, $producto->id_meli, '0');
                        ...
                        if (count($stocks) > 1) {
                            ...
                            $stocks_1 = Util::getStock($producto->id, 'all');
                            $variationsInit = $this->getVariations($producto->id_meli);                                                                                                               
                            $variations = $this->updateVariations($stocks_1, $variationsInit, $precio->precio_db, $producto->id, $producto->id_meli, '0');
                            ...
                            $item = array(
                                ...
                                "condition" => $producto->estado=='Usado'?"used":"new",
                                "video_id" => $producto->id_video,
                                "available_quantity" => (int)$stocks[0]->stock,
                                "tags" => array( "immediate_payment"),
                                "shipping" => array(
                                    "mode" => "me2",
                                    "local_pick_up" => true,
                    ...
                    if ($resultDescripcion['httpCode'] == 200) {
                        ...
                        SincroMeliLog::alta($resultDescripcion,$producto->id);
                        $aResult['status'] = 1;
                    ...
                    if (count($variation->attribute_combinations) == 1) {
                        ...
                        if ($stock->nombreColor == $value_name) {
                            ...
                            if (count($imagenesColor) == 0) {
                                ...
                                $aResult['msg'] = "Debe cargar como mínimo una foto por cada color.";
                                $res = json_decode('{"body":{"message":"Debe cargar como mínimo una foto por cada color.","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                                SincroMeliLog::alta($res,$id_producto);
                                return $aResult;
                            }
                            $pictures_color = array();
                            foreach ($imagenesColor as $imagenes) {
...
//                                $variation->talle = $stock->nombreTalle;
                ...
                if ($b == 1) {
                    ...
                    SincroMeliLog::alta($status,$id_producto);
                    unset($variations[$i]);
                }
                $i++;
            }            
                                ...
                                if (isset($nombreColor[1])) {
                                    ...
                                    if ($nombreColor[1] == $color_secundario) {
                                        ...
                                        if (count($imagenesColor) == 0) {
                                            $aResult['status'] = 1;
                                            $aResult['msg'] = "Debe cargar como mínimo una foto por cada color.";
                                            $res = json_decode('{"body":{"message":"Invalid token","cause":[],"error":"not_found","status":401},"httpCode":401}',true);
                                            SincroMeliLog::alta($res,$id_producto);
                                            return $aResult;
                                        }
                                        $pictures_color = array();
