...
class Controller extends BaseController
{
	...
	protected function deleteImgs($resourceId) {
		...
		if (!$imgs->isEmpty()) {
			foreach ($imgs as $img) {
				@unlink(\config('appCustom.UPLOADS_BE') . $this->resource . '/' . $img->imagen_file);
				@unlink(\config('appCustom.UPLOADS_BE') . $this->resource . '/' . '100_' . $img->imagen_file);
				@unlink(\config('appCustom.UPLOADS_BE') . $this->resource . '/' . '300_' . $img->imagen_file);
