...
class CartController extends Controller
	{
		...
		/*	SDK::setClientId(env('MP_APP_ID'));
           		SDK::setClientSecret(env('MP_APP_SECRET'));*/
        ...
        if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
            ...
            if(isset($item['envio']['tipo']['id_tipo_envio'])){
                $tipo_envio = TipoEnvio::where('id_tipo',$item['envio']['tipo']['id_tipo_envio'])->first();
                if (!$tipo_envio) $tipo_envio = TipoEnvio::where('id_tipo_envio',$item['envio']['tipo']['id_tipo_envio'])->first();
            ...
            if(count($item['carrito'])>1){
                $nombre_producto = 'Marcantonio Deportes - Compra online';
                $foto_prod = '';
                $tamano_paquete = '';
            }else{
		...
		public function notificaciones_meli(Request $request){
            \Log::info('notificaciones_meli cart');
            ...
            if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
                ...
                if ($data["topic"] == 'payment') {
                    ...
                    \Log::info('Payment Info' . print_r($payment_info, true));
                ...
                if ($merchant_order_info["status"] == 200) {
                    ...
                    if (!$pedido) {
							if(strlen($id_pedido) < 7){
								$pedido = new Pedidos;
								$pedido->id_pedido = $id_pedido;
								$pedido->save();
							}else{
								$aResult['status'] = 1;
								$aResult['msg'] = 'Notificacion no correspondiente a mercado pago';
								return response()->json($aResult);
							}
						}
					...
					if($pedido->payment_id == null ||$pedido->payment_id == '' || $pedido->estado == 'pending' || $pedido->estado=='rejected'){
						...
						if($pedido->id_tipo_envio!=-1){
                                ...
                                $pedido->precio_venta = $total-$pedido->costo_envio;
                            ...
                            if($pedido->id_tipo_envio==1){ // Para el caso de MercadoEnvios el total recibido no incluye el costo de envio
						...
						if ($data_payments["shipping_cost"]==0) {	//No pidio por mercado Envios
                            ...
                            if ($tipo_envio > 1) {
                                ...
                                if (isset($marchant_orden_response["shipments"][0]["receiver_address"]["state"]["name"])) {
                                    $provincias = Provincias::select('id_provincia')->where('provincia', $marchant_orden_response["shipments"][0]["receiver_address"]["state"]["name"])->first();
                                    if ($provincias) {
		...
		public function todoPago(Request $request)
		{
			...
			if ($rta['StatusCode'] == -1) {
				...
				$pedido->save();
		            ...
		            $notificacion = new PedidosNotificaciones();
		            $notificacion->id_pedido = $pedido->id_pedido;
		            $notificacion->texto = json_encode(array( 'optionsSAR_comercio' => $optionsSAR_comercio, 'optionsSAR_operacion' => $optionsSAR_operacion,));
		            $notificacion->status = $pedido->estado;
		            $notificacion->more_info = json_encode($rta);
		            $notificacion->emisor = 'Todo Pago Alta';
		            $notificacion->save();
			$aResult['data'] = $rta;
			return response()->json($aResult);
		}
...
\Log::debug('Oprions GAA '.print_r($optionsGAA,true));
			//común a todas los métodos
			$http_header = array('Authorization'=>\config('appCustom.TP_API_KEY'));
			...
			if ($rta2['StatusCode'] != -1) {
				...
				if ($pedido->id_direccion_envio) {
					...
					switch ($tipo_envio->empresa) {
						case 'Andreani':
							$envio =app('App\Http\Controllers\WebServices\AndreaniController')->altaEnvio($pedido->id_pedido);
							PedidosProductos::where('id_pedido', $pedido->id_pedido)
									->update(['id_tipo_envio' => $pedido->id_tipo_envio,'id_tipo' => $tipo_envio->id_tipo, 'precio_envio' => $pedido->costo_envio]);//seteo el tipo de envio en productos
				...
				Cart::reservarStock($pedido->id_pedido, $pedido->id_sucursal);
                // El pedido se ha pagado, envio mail al administrador
