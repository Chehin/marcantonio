<?php
	...
	class AuthController extends Controller
	{
		...
		public function registro(Request $request){
            ...
            // validar captcha
/*            $secret = \env('CAPTCHA_SECRECT_KEY');

            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'https://www.google.com/recaptcha/api/siteverify?remoteip='.$_SERVER['REMOTE_ADDR'].'&secret='.$secret.'&response='.$data['g-recaptcha-response']);
            ...
            if($response->success == false){
                ...
                $aResult['data']['msg'] = 'Recaptcha not found';
...
*/
			...
			if ($validator->fails()) {
             ...
             //   \Log::debug(' Registro Cliente: '.print_r($cliente,true));
			}
		...
		public function direcciones(Request $request){
			...
			if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
				...
				->leftJoin('provincias','provincias.id_provincia','=','pedidos_direcciones.id_provincia')
			...
			if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
				...
				if($id){
					...
					if($direccion){
						...
						$localidades = array('' => 'Seleccionar ciudad') + Localidades::where('cod_provincia','=',$provincia->codigo)->orderBy('nombre')->lists('nombre','id')->toArray();
						$aResult['data']['localidades'] = $localidades;
					}
				}
				$provincias = Provincias::orderBy('provincia')->lists('provincia','id_provincia');
		...
		public function getLocalidad(Request $request){
            ...
            if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
                ...
                if ($prov) {
                    $localidades = Localidades::select('id','nombre')->where('cod_provincia','=',$prov->codigo)->orderBy('nombre')->get();
