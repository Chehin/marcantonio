...
/*		array_walk($aOItems, function(&$val,$key)use($resource){
	...
	static function getPrecios($id, $id_moneda) {
		...
		->where('id_moneda', $id_moneda)->first();
		if($precio){
			//si tiene descuento, va sobre el precio de lista
			if(isset($precio->descuento) && $precio->precio_lista>0){
	...
	static function getColorTalles($id, $id_color = 0, $id_marca=0, $id_genero=0,$id_rubro=0) {
		...
		array_walk($codigo, function(&$val,$key)use($id){
			...
			->leftJoin('conf_talles','conf_talles.id','=','inv_producto_codigo_stock.id_talle')
             ->leftJoin('inv_producto_stock_sucursal','id_codigo_stock','=','inv_producto_codigo_stock.id')
...
/*				array_walk($talles, function(&$val,$key)use($id_marca,$id_genero,$talles,$id_rubro){
    ...
    static function getStockTotal($id) {
        $stockTotal = SucursalesStock::
        selectRaw('codigo.id_producto , producto.nombre, SUM(inv_producto_stock_sucursal.stock) as stock')
            ...
            ->where('codigo.id_producto', $id)
            ->groupBy('codigo.id_producto')
            ->first();

        $stocks_x_talle = SucursalesStock::
        selectRaw('codigo.id_talle , talle.nombre AS talle , SUM(inv_producto_stock_sucursal.stock) as stock')
            ->leftJoin('inv_producto_codigo_stock AS codigo','codigo.id','=','inv_producto_stock_sucursal.id_codigo_stock')
            ->leftJoin('conf_talles AS talle','talle.id','=','codigo.id_talle')
            ->leftJoin('editorial_notas','editorial_notas.id_nota', '=', 'inv_producto_stock_sucursal.id_sucursal' )
            ->where('editorial_notas.habilitado', 1)
            ->where('inv_producto_stock_sucursal.stock', '>',0)
            ->where('codigo.id_producto', $id)
            ->orderBy('talle.nombre','asc')
            ->groupBy('codigo.id_talle')
            ->get();

        $data = array(
            'total' => $stockTotal->stock,
            'talles'=> array()
        );

        foreach($stocks_x_talle as $stock_talle){
            $stocks_talle_x_color = SucursalesStock::
            selectRaw('codigo.codigo , codigo.id_color , color.nombre AS color , SUM(inv_producto_stock_sucursal.stock) as total')
                ->leftJoin('inv_producto_codigo_stock AS codigo','codigo.id','=','inv_producto_stock_sucursal.id_codigo_stock')
                ->leftJoin('inv_productos AS producto','producto.id','=','codigo.id_producto')
                ->leftJoin('conf_colores AS color','color.id','=','codigo.id_color')
                ->leftJoin('conf_talles AS talle','talle.id','=','codigo.id_talle')
                ->leftJoin('editorial_notas','editorial_notas.id_nota', '=', 'inv_producto_stock_sucursal.id_sucursal' )
                ->where('editorial_notas.habilitado', 1)
                ->where('producto.id', $id)
                ->where('codigo.id_talle', $stock_talle->id_talle)
                ->groupBy('codigo.id_talle','codigo.id_color')
                ->get()->toArray();

            $talle = array(
                'id_talle' => $stock_talle->id_talle,
                'talle' => $stock_talle->talle,
                'total' => $stock_talle->stock,
                'colores'=> $stocks_talle_x_color
            );

            array_push($data['talles'],$talle);
        }
