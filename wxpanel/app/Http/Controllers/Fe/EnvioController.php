...
class EnvioController extends Controller
{
	...
	public function getTipoEnvio(Request $request){
        ...
        if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
			...
			$alto = $ancho = $largo = $peso = $volumenTotal = 0;
            foreach ($pedidos as $pedido) {
                $cantidad = (int)$pedido['cantidad'];
                $item = Productos::find($pedido['id_producto']);
				$volumenTotal = $volumenTotal + ($item->alto*$item->ancho*$item->largo*$cantidad);
            ...
            if($mercado_envio){
                \Log::debug('Dimensiones'.$alto."x".$ancho."x".$largo.",".$peso);
			...
			if($andreani){
			    \Log::debug('pasa');
				$volumenAndreani = $alto*$ancho*$largo;
				
				//Peso de g a Kg
				$pesoAndreani = $peso / 1000;
				
				if($volumenTotal>0){
					$volumenAndreani = $volumenTotal;
				}
					...
					foreach ($tipo_envio as $tipo) {
						...
						if($response->isValid()){
							...
							if($envioGratis){
								...
								$tarifa = $mensaje->CotizarEnvioResult->Tarifa;
							}
							...
							array_push($respuesta,$data);
							\Log::debug('rta '.print_r($data,true));
						}
					}
				}
			}
	...
	public function getSucursalEnvio(Request $request){
		...
		if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
			...
			->where('id_edicion',-10) //\config('appCustom.MOD_SUCURSALES_FILTER')
			->where('habilitado',1)
			...
			array_walk($sucursales, function(&$val,$key)use($carrito){
				...
				if($stock_sucursal){
                   ...
                   // \Log::debug('data sucursal'.$stock_sucursal);
				}else{
					$val['disponible'] = 0;
	...
	public function consultaCostoEnvio(Request $request){
		...
		if ($this->user->hasAccess($this->resource . '.view') && $this->filterNote) {
			...
			$respuesta = array();
				$volumenTotal = $alto = $ancho = $largo = $peso = 0;
			            foreach ($pedidos as $pedido) {
			                $cantidad = (int)$pedido['cantidad'];
			                $item = Productos::find($pedido['id_producto']);
					
					$volumenTotal = $volumenTotal + ($item->alto*$item->ancho*$item->largo*$cantidad);
			                $alto = round(pow($volumenTotal, 1/3));
			                $ancho = round(pow($volumenTotal, 1/3));
			                $largo = round(pow($volumenTotal, 1/3));
			                $peso = $peso + ($item->peso*$cantidad);
				    ...
				    if($mercado_envio){
			                $tamano_paquete = $alto."x".$ancho."x".$largo.",".$peso;
			                \Log::debug('Tamano paquete'.$alto."x".$ancho."x".$largo.",".$peso);
			                // Verfico que el articulo no supere las medidas estandar para Mercado Envíos
			                $params = array(
			                    "dimensions" => $tamano_paquete,
