...
$('select[name=direccion_envio]').on('change', function () {
    ...
    $('#nombre_retira').attr('disabled', 'disabled');
...
$('select[name=tipo_envio]').on('change', function () {
    ...
    if ($(this).val()) {
        ...
        $('#nombre_retira').removeAttr('disabled');
...
function add_to_cart(_this) {
    ...
    var id_color = parseInt($(_this).parents('form').find('input[name=id_color]').val());
...
function cart_update(id, method, cantidad, id_color = 0, id_talle = 0, nombre = '') {
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            $('.shoppingcart-inner').find('.cart-total span.ctn').html(data.cantidad);
