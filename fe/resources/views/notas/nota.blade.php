...
@section('content')
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{route('notas',['id' => $nota['id_seccion'],'name' => str_slug($nota['seccion'])])}}">{{ $nota['seccion'] }}</a></li>
            <li class="breadcrumb-item"><a href="{{route('nota',['id' => $nota['id_nota'],'name' => str_slug($nota['titulo'])])}}">{{ $nota['titulo'] }}</a></li>
        </ol>
    </nav>
                        ...
                        <div class="entry-photo pull-left col-sm-5">
                            <div class="row">
                                ...
                                <figure class="mb20 {{ $i>0?'col-sm-4':'col-sm-12' }}">
                                    <a href="{{ env('URL_BASE').'uploads/news/'.$foto['imagen_file'] }}" class="foto_modal">
                                        <img src="{{ env('URL_BASE').'uploads/news/'.$foto['imagen_file'] }}" alt="{{$foto['epigrafe']}}">
                                    </a>
                                </figure>
                                @php $i++; @endphp
                                @endforeach
