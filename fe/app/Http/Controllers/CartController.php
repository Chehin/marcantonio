...
class CartController extends Controller
{
        ...
        if($method=='add'){
            ...
            try {
                ...
                if ($dataC['status'] == 0){
                  ...
                  //  \Log::debug('Carrito: '.print_r($_SESSION['carrito'],true));
                }
            ...
            if(!$_SESSION['carrito'] || $_SESSION['id_user']){
               ...
               // \Log::debug('carrito: '.print_r($carrito,true));
            }else{
        ...
        if($carrito){
            ...
            if($carrito['error']!=1){
                foreach($carrito['carrito'] as $cart){
                    \Log::debug(print_r($carrito,true));
                    if($cart['fotos'][0]['imagen_file'] != ""){
                        $foto = env('URL_BASE').'uploads/productos/'.$cart['fotos'][0]['imagen_file'];
                    }else{
                        $foto = env('URL_BASE').'uploads/productos/prodnodisponible.png';
                    }

                        $_div .= '<tr id="product_id_'.$cart['id_producto'].'_'.$cart['id_color'].'_'.$cart['talle'].'">';
                            $_div .= '<td class="product-actions">';
                            $_div .= '<a href="javascript:void(0);" class="remove" title="Eliminar producto" onclick="remove_to_cart('.$cart['id_pedido_producto'].')"><i class="fa fa-times"></i></a>';
                            $_div .= '</td>';
                            $_div .= '<td class="product-thumbnail">';
                            $_div .= '<a href="'.route('producto',['id' => $cart['id_producto'],'name' => str_slug($cart['titulo'])]) .'" title="'.$cart['titulo'].'"><img src="'.$foto.'" alt="'.$cart['titulo'].'" width="150px" height="150px" class="img-fluid"></a>';
                            $_div .= '</td>';

                            $_div .= '<td class="product-name" style="width:70%">';
                            $_div .= '<a href="'.route('producto',['id' => $cart['id_producto'],'name' => str_slug($cart['titulo'])]) .'" title="'.$cart['titulo'].'"><small>'.$cart['titulo'].'</small><br> <span class="amount"><strong>'.$cart['cantidad'].' x '.$cart['moneda'].$cart['precio']['precio'].'</strong></span></a>';
                            $_div .= '</td>';
                            //$_div .= '<hr style="width:80%;">';
                        $_div .= '</tr>';
                ...
                if($_div==''){
                    $_div = '<tr><td colspan="4" align="center"><p><strong>No hay productos</strong></p></td></tr>';
                }else{
                    $_SESSION['carrito'] = $carrito;
                }
                $array = array(
            ...
            $_div = '<tr><td colspan="5"><p><strong>No hay productos</strong></p></td></tr>';
            $array = array(
                'data' => $_div,
			...
			switch ($id) {
                ...
                if($_SESSION['id_user']!=0){
                    if(isset ($request['procesar_pedido']) ){
                        if($request['envio_db']!='' && $request['id_tipo_envio']!='' && $request['id_direccion_envio']!='' && ($request['id_direccion_envio']!='' || $request['id_sucursal'] !='')){
                        ...
                        return view('cliente.login', ['data'=>'','pageTitle'=> env('SITE_NAME')." - Ingresar"]);
                    }
				break;
                ...
                case 2:
                 //datos del pedido
                    ...
                    try {
                        ...
                        \Log::info('Preference '.print_r($data,true));
                    } catch (RequestException $e) {
                        Log::error(Psr7\str($e->getRequest()));
                        if ($e->hasResponse()) {
                            Log::error($e->getMessage());
					...
					return view('cart.procesar_pedido_2', ['data'=> $carrito,'pageTitle'=>$pageTitle])->with('preference_data', $preference_data);
					break;*/
                default:
					# code...
					break;
    ...
    public function costoEnvio(Request $request, Api $api){
        ...
        $array_te = array(
            ...
            'edicion' => 'pedidos',
            'id' => $request['id'],
            'pedido' => $_SESSION['carrito']['carrito'],
